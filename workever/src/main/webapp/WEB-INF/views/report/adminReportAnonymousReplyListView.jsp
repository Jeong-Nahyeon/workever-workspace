		          ...
		          <div class="card"> 
		            ...
		            <div class="card-header board-header-area">
		              <div class="col-sm-6 board-header-area-left">
		                <div class="btn-group">
		                  <button type="button" class="btn btn-sm btn-default" onclick="location.href='list.bo';">게시글 관리</button>
		                  <button type="button" class="btn btn-sm anonymous-reply-btn" onclick="location.href='list.re';">댓글 관리</button>
		                </div>
		              </div>
		                  ...
		                  <c:choose>
				              ...
				              <c:otherwise>
				                  ...
				                  <tr>
				                    ...
				                    <td class="report-no">${ r.reportNo }</td>
				                    <td>${ r.reportGroup }</td>
				                    <td class="report-title">익명 댓글 신고</td>
				                    <td class="report-ref-no">${ r.reportRefNo }</td>
				                    <td>
				                    	<c:if test="${ r.crStatus eq 'B' }">
			                    				완료
			                    		</c:if>  
				                    </td>
				                    <td>${ r.reportDate }</td>
		              ...
		              <div class="paging-area">
						<c:if test="${ not empty list }">
	              			<c:if test="${ pi.currentPage ne 1 }">
		                		<a class="btn" href="list.re?cpage=${ pi.currentPage - 1 }">&lt;</a>
		                	</c:if>
	...
	<script>
		$(function(){
					...
					success:function(report){
						...
						if(report != null){
						  ...
						  let detail  = "<thead><tr><th>신고 작성 사원</th>"
									  ...
									  + "href='detail.abo?abno=" + report.crRefNo + "#" + report.reportRefNo + "'>링크이동</a></td></tr>"
									  + "<tr><th colspan='4'>신고 내용</th></tr></thead>"
									  + "<tbody><tr><td colspan='4'><div id='report-content'>";
									  
									  if(report.reportContent != null){
										  
										  detail += report.reportContent + "<br><br><br>";
										  
									  }
									  
									  
									  
							 detail += "<h6><b>&lt; 신고대상 댓글 내용 &gt;</b></h6>"
							 		 + "<p> 댓글번호 : " + report.reportRefNo + "<br>"
							 		 + "댓글내용 : <br>"
							 		 + report.crContent
							 		 + "</p>"	
								 	 + "</div></td></tr></tbody>";
							...
							let blind = "";
							if(report.crStatus == "B"){ // 블라인드 처리 후에 보이는 버튼 => 블라인드 해제 버튼
							...
							$("#postForm input[name=crNo]").val(report.reportRefNo);
			...
			$(document).on("click", "#blind-btn", function(){
				...
				$.ajax({
					url:"blind.re",
					data:{crNo:$("#postForm input[name=crNo]").val()},
