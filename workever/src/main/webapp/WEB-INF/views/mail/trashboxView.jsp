		          ...
		          <div class="card"> 
		            ...
		            <div class="card-header mail-btns-area"> 
		              <div class="col-sm-6 mail-btns-area-left">
	                      <button id="inbox-btn" class="btn btn-sm btn-default">받은메일함 이동</button>
	                    </div>
		              ...
		              <div class="paging-area">
						<c:if test="${ not empty list }">
	              			<c:if test="${ pi.currentPage ne 1 }">
		                		<a class="btn" href="trashbox.mail?cpage=${ pi.currentPage - 1 }">&lt;</a>
		                	</c:if>
			                
			                <c:forEach var="p" begin="${ pi.startPage }" end="${ pi.maxPage }">
	                			<c:choose>
				                	<c:when test="${ pi.currentPage eq p }">
		                				<a class="btn active-color" href="trashbox.mail?cpage=${ p }">${ p }</a>
		                			</c:when>
		                			<c:otherwise>
		                				<a class="btn" href="trashbox.mail?cpage=${ p }">${ p }</a>
		                			</c:otherwise>
	                			</c:choose>
			                </c:forEach>
	...
	<script>
		$(function(){
			...
			$(".mail-title").click(function(){
				...
				location.href = "incoming.mail?mrNo=" + mrNo + "&mrMsNo="+ mrMsNo + "&mrRead=" + mrRead + "&from=t";
			...
			$("#delete-btn").click(function(){
				...
				$("input[name=checkedNo]:checked").each(function(){
					...
					mrNoArr.push($(this).next().val());
					mrMsNoArr.push($(this).parent().next().next().next().find("input[name=mrMsNo]").val());
							...
							success:function(result){
								...
								if(result == "success"){
									...
									$("#confirm-modal").modal("hide"); 
					 				...
					 				$("#alert-message").text("성공적으로 삭제되었습니다(복구 불가)");
									$("#alert-message-modal").modal({backdrop: "static"});
								...
								console.log("메일 목록 완전 삭제용 ajax 통신 실패");	
			...
			$("#inbox-btn").click(function(){
				...
				// 이동할 메일 체크 하지 않고 받은메일함 이동 버튼 클릭 시
				if(checkedArr.length == 0){
					
					alert("메일 선택 후 해당 버튼을 눌러주세요.");
					
				}else{ // 이동할 메일 체크 후 받은메일함 이동 버튼 클릭 시
					
					$.ajax({
						url:"toinbox.mail",
						dataType:"json",
						type:"post",
						data:{deliveryList:checkedArr},
						success:function(result){
							...
							if(result == "success"){
								...
								$("#confirm-modal").modal("hide"); 
				 				...
				 				$("#alert-message").text("성공적으로 이동되었습니다");
								$("#alert-message-modal").modal({backdrop: "static"});
